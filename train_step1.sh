...
elif [[ "syn_test" == "$1" ]]; then
    echo "start with syn_test"
  python train.py  --dataset_name co3d_proj2  --root_dir  DATASET/synthetic_edit/lego --N_importance 64 --img_wh 400 400 --num_epochs 2000 --batch_size 1  --optimizer adam --lr 2e-4  --lr_scheduler steplr --decay_step 500 1000 --decay_gamma 0.5  --exp_name syn_test --with_ref --patch_size $patch_size --sW 6 --sH 6 --proj_weight 1 --depth_smooth_weight 0  --dis_weight 0 --num_gpus $n_gpu --load_depth --depth_type nerf --model sinnerf --depth_weight 8 --vit_weight 10 --scan 4
